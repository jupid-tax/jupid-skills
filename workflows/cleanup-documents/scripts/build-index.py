...
Usage:
    ...
    python3 build-index.py manifest.json --csv document-index.csv --moves moves-result-<ts>.json
...
COLUMNS = [
    ...
    ("Amount", "amount"),
    ("Currency", "currency"),
    ("Basis for Classification", "basis"),
...
def load_rows(manifest_path: Path, moves_path: Path | None) -> list[dict]:
    ...
    for entry in manifest:
        ...
        row["needs_review"] = "YES" if entry.get("needs_review") else ""
        rows.append(row)
    return rows


def summary_counts(rows: list[dict]) -> list[tuple[str, int]]:
    counts: dict[str, int] = {}
    for row in rows:
...
def write_xlsx(rows: list[dict], out: Path) -> bool:
    try:
    ...
    for row in rows:
        ws.append([row.get(key) if row.get(key) is not None else ""
                   for _, key in COLUMNS])
    widths = [40, 45, 24, 16, 24, 12, 12, 9, 60, 12, 30, 14, 12]
    ...
    summary.append(["Needs review", sum(1 for r in rows if r["needs_review"])])
    summary.column_dimensions["A"].width = 30

    wb.save(out)
    print(f"Excel index → {out} ({len(rows)} rows + Summary sheet)")
    return True
...
def main() -> int:
    ...
    rows = load_rows(Path(args.manifest), Path(args.moves) if args.moves else None)
    if not rows:
        print("manifest is empty — nothing to index", file=sys.stderr)
        return 1

    if args.xlsx:
        if not write_xlsx(rows, Path(args.xlsx)):
